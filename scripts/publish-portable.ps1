...
try{
    ...
    $health=Start-Process -FilePath (Join-Path $target 'HomeKTV.exe') -ArgumentList @('--portable-media-smoke','--health-check') -WorkingDirectory $target -WindowStyle Hidden -PassThru;if(-not $health.WaitForExit(240000)){Stop-Process -Id $health.Id -Force -ErrorAction SilentlyContinue;$health.WaitForExit(10000)|Out-Null;throw '便携版 MV/音频/幻灯片混合播放健康检查超时。'};if($health.ExitCode -ne 0){throw "便携版健康检查失败：$($health.ExitCode)"};$health.Dispose()
