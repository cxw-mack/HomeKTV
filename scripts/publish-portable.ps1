...
try{
    ...
    $health=Start-Process -FilePath (Join-Path $target 'HomeKTV.exe') -ArgumentList @('--import-demo','--health-check') -WorkingDirectory $target -WindowStyle Hidden -PassThru;if(-not $health.WaitForExit(60000)){Stop-Process -Id $health.Id -Force;throw '便携版导入与初始化健康检查超时。'};if($health.ExitCode -ne 0){throw "便携版健康检查失败：$($health.ExitCode)"};$health.Dispose()
