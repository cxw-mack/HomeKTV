...
public sealed class AudioImportFeatureTests
{
    ...
    [Fact] public async Task SongFolderScanImportsVideoLyricsAndAccompanimentAsOneSong()
    {
        await using var fixture=await FeatureTestRoot.CreateAsync();var library=Path.Combine(fixture.Root,"乐库");var folder=Path.Combine(library,"F.I.R飞儿乐团 - 千年之恋");Directory.CreateDirectory(folder);
        var generatedVideo=await GenerateVideoAsync(fixture,"folder-source.mp4");var generatedAudio=await fixture.GenerateMediaAsync("folder-backing.wav","-c:a pcm_s16le");var video=Path.Combine(folder,"F.I.R飞儿乐团 - 千年之恋.mp4");var accompaniment=Path.Combine(folder,"F.I.R飞儿乐团 - 千年之恋 - 伴奏.wav");File.Move(generatedVideo,video);File.Move(generatedAudio,accompaniment);var lyrics=Path.Combine(folder,"F.I.R飞儿乐团 - 千年之恋.lrc");await File.WriteAllTextAsync(lyrics,"[00:00.00]第一句\n[00:01.00]第二句");
        var candidate=Assert.Single(MediaFolderScanner.Scan(library,true));var result=await CreateImporter(fixture).ImportAsync(new(candidate.VideoPath,candidate.Title,candidate.Artist,LyricPath:candidate.LyricPath,AccompanimentMediaPath:candidate.AccompanimentMediaPath));
        var song=Assert.IsType<Song>(result.Song);Assert.Equal("F.I.R飞儿乐团",song.ArtistDisplayName);Assert.Equal("千年之恋",song.Title);Assert.Equal(SongMediaType.VideoWithExternalAudio,song.MediaType);Assert.NotNull(song.LyricRelativePath);Assert.NotNull(song.AccompanimentAudioRelativePath);Assert.True(File.Exists(fixture.Paths.Resolve(song.LyricRelativePath!)));Assert.True(File.Exists(fixture.Paths.Resolve(song.AccompanimentAudioRelativePath!)));
    }
