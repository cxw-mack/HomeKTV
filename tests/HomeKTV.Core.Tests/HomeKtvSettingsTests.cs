...
public sealed class HomeKtvSettingsTests
{
    ...
    public void LyricsDisplaySettingsAreNormalizedAndRetainVisibilityPreference()
    {
        ...
        HomeKtvSettingsValidator.Normalize(settings);
        Assert.False(settings.Lyrics.LastVisible);Assert.Equal("F7",settings.Lyrics.ToggleShortcutKey);Assert.Equal("Microsoft YaHei UI",settings.Lyrics.FontFamily);Assert.Equal(96,settings.Lyrics.FontSize);Assert.Equal(0,settings.Lyrics.BackgroundOpacity);Assert.Equal(400,settings.Lyrics.HorizontalMargin);Assert.Equal(0,settings.Lyrics.LineSpacing);Assert.Equal(8,settings.Lyrics.OutlineThickness);Assert.Equal(HomeKTV.Core.Models.LyricsDisplayMode.Karaoke,settings.Lyrics.DisplayMode);Assert.Equal(HomeKTV.Core.Models.LyricsOverlayPosition.Bottom,settings.Lyrics.Position);
    }
}
