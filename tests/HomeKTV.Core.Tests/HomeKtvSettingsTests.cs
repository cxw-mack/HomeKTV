...
public sealed class HomeKtvSettingsTests
{
    ...
    {
        var settings=new HomeKtvSettings{MediaRoot=Path.GetFullPath("outside"),IdleBackgroundRelativePath="../outside",ServerPort=-1,AdministratorPin="",DefaultVolume=999,AccompanimentVolumeGain=9,AutomaticBackupHours=0,LogLevel="invalid"};
        HomeKtvSettingsValidator.Normalize(settings);
        Assert.Equal("Media",settings.MediaRoot);Assert.Equal("Media/Backgrounds",settings.IdleBackgroundRelativePath);Assert.Equal(1024,settings.ServerPort);Assert.Equal("8888",settings.AdministratorPin);Assert.Equal(125,settings.DefaultVolume);Assert.Equal(1.5,settings.AccompanimentVolumeGain);Assert.Equal(1,settings.AutomaticBackupHours);Assert.Equal("Information",settings.LogLevel);
    }
    ...
    [Fact]
    public void IncorrectLegacyAccompanimentBoostMigratesToFortyPercent()
    {
        var settings=new HomeKtvSettings{SchemaVersion=6,AccompanimentVolumeGain=2.5};
        HomeKtvSettingsValidator.Normalize(settings);
        Assert.Equal(.4,settings.AccompanimentVolumeGain);Assert.Equal(HomeKtvSettings.CurrentSchemaVersion,settings.SchemaVersion);
    }
}
