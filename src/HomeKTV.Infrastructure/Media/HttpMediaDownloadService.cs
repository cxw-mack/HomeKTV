...
public sealed class HttpMediaDownloadService(PortablePaths paths)
{
    ...
    {
        if(!Uri.TryCreate(url,UriKind.Absolute,out var uri)||uri.Scheme is not ("http" or "https"))throw new ArgumentException("只支持 HTTP 或 HTTPS 直接媒体地址。",nameof(url));
        paths.EnsureDirectories();var extension=Path.GetExtension(uri.AbsolutePath).ToLowerInvariant();
        if(extension is not (".mp4" or ".mkv" or ".avi" or ".mov" or ".m4v" or ".webm"))throw new InvalidDataException("直链地址必须以受支持的视频扩展名结尾。");
        var target=Path.Combine(paths.Downloads,Guid.NewGuid().ToString("N")+extension);var temporary=target+".downloading";
        try
        {
            using var request=new HttpRequestMessage(HttpMethod.Get,uri);request.Headers.UserAgent.Add(new ProductInfoHeaderValue("HomeKTV","1.0"));
            ...
            var mediaType=response.Content.Headers.ContentType?.MediaType;if(mediaType is not null&&!mediaType.StartsWith("video/",StringComparison.OrdinalIgnoreCase)&&mediaType!="application/octet-stream")throw new InvalidDataException($"服务器返回的内容类型不是视频：{mediaType}");
            var total=response.Content.Headers.ContentLength;await using var input=await response.Content.ReadAsStreamAsync(cancellationToken);
            ...
            await output.FlushAsync(cancellationToken);if(received==0)throw new InvalidDataException("下载结果为空。");if(total is >0&&received!=total)throw new InvalidDataException("下载未完成，文件长度与服务器声明不一致。");
