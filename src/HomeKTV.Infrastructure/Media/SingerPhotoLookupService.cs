...
public sealed class SingerPhotoLookupService
{
    ...
    public SingerPhotoLookupService(PortablePaths paths)
    {
        _paths=paths;
    ...
    {
        var path=Path.Combine(_paths.SingerCovers,GetCacheKey(singerName)+".miss");
        return File.Exists(path)&&File.GetLastWriteTimeUtc(path)>DateTime.UtcNow.AddDays(-30);
    }

    public async Task<SingerPhotoLookupResult?> LookupAndCacheAsync(string singerName,CancellationToken cancellationToken=default)
    {
        ...
        if(HasRecentMiss(normalized))return null;
    ...
    private static async Task<SingerPhotoSource?> FindNetEasePhotoAsync(string singerName,CancellationToken cancellationToken)
    {
        var url="https://music.163.com/api/search/get/web?s="+Uri.EscapeDataString(singerName)+"&type=100&limit=8&offset=0";
        ...
        if(!document.RootElement.TryGetProperty("result",out var result)||!result.TryGetProperty("artists",out var artists))return null;
        ...
        foreach(var artist in artists.EnumerateArray())
        {
            ...
            var primaryMatch=!string.IsNullOrWhiteSpace(name)&&NamesMatch(name,singerName);
            var aliasMatch=aliases.Any(alias=>NamesMatch(alias,singerName));
            ...
            var score=(primaryMatch?100:90)+Math.Min(albumSize,50)*2+Math.Min(musicSize,100)/10;
            matches.Add((score,id,imageUrl));
        }
        var selected=matches.OrderByDescending(match=>match.Score).FirstOrDefault();
        return selected.ImageUrl is null?null:new(selected.Id,selected.ImageUrl,$"https://music.163.com/#/artist?id={selected.Id}","image/jpeg","网易云音乐公开艺人图片","网易云音乐");
    }
    ...
    private static async Task<string?> GetCommonsFileNameAsync(string entityId,CancellationToken cancellationToken)
    {
    ...
    private static async Task<(string Url,string DescriptionUrl,string Mime,string License,string Attribution)?> GetCommonsImageAsync(string fileName,CancellationToken cancellationToken)
    {
    ...
    {
        File.WriteAllText(Path.Combine(_paths.SingerCovers,GetCacheKey(singerName)+".miss"),DateTimeOffset.UtcNow.ToString("O"));
        return null;
    }
